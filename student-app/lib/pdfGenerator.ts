...
<body>
    ...
    <div class="questions">
        ${results.map((item, index) => {
            ...
            <div class="question-card">
                <div class="q-header">
                    <span class="q-num">Result ${index + 1}</span>
                </div>
                
                ${(item.solution || item.summary) ? `
                    <div class="solution-highlight" style="margin-top: 0; margin-bottom: 20px; border-color: #cbd5e1; background: #f8fafc;">
                        <div style="font-weight: 800; color: #0f172a; font-size: 18px;">${parseMarkdown(item.solution || item.summary)}</div>
                    </div>
                ` : ''}
