...
class LecturerExamQuestions extends Component
{
    ...
    /**
     * Handle results generated by client-side Puter.js
     * @param array $questions The array of question objects (including image URLs)
     */
    public function handleAIResults($questions)
    {
        try {
            if (empty($questions) || !is_array($questions)) {
                $this->toastError('No questions received from AI.', 'Error');
                return;
            }

            // Assign unique IDs and normalize
            $this->aiGeneratedQuestions = array_map(function ($q, $index) {
                // Generate a robust unique ID
                $q['id'] = 'ai_' . uniqid() . '_' . $index;
                $q['marks'] = 1;

                // Handle MCQ letter-to-value mapping
                if (isset($q['question_type']) && $q['question_type'] === 'multiple_choice' && isset($q['options']) && isset($q['correct_answer'])) {
                    $letterMap = ['A' => 0, 'B' => 1, 'C' => 2, 'D' => 3, 'E' => 4];
                    $ans = trim($q['correct_answer']);
                    
                    // If the answer is just a letter "A", map it to the option text
                    if (strlen($ans) === 1 && isset($letterMap[strtoupper($ans)])) {
                        $idx = $letterMap[strtoupper($ans)];
                        $q['correct_answer'] = $q['options'][$idx] ?? $ans;
                    }
                }
            ...
            $this->isGeneratingQuestions = false;
            $this->generationProgress = 100;
            $this->toastSuccess(count($questions) . ' questions received!', 'Success');
            
            // Critical: Dispatch render-math for immediate rendering
            $this->dispatch('render-math');
            
        } catch (\Exception $e) {
            \Log::error('AI Handoff Error: ' . $e->getMessage());
            $this->toastError('Failed to process AI results.', 'Error');
        }
    }
