...
class TestEmailPreview extends Component
{
    ...
    private function sendInvoiceEmail()
    {
        ...
        try {
            Mail::mailer(config('mail.default'))->to($this->testEmail)->send(new InvoiceEmail($invoice, $this->testEmail));
    ...
    private function sendWelcomeEmail()
    {
        ...
        try {
            Mail::mailer(config('mail.default'))->to($this->testEmail)->send(new WelcomeEmail($user, $schoolName));
    ...
    private function sendUpgradeEmail()
    {
        ...
        try {
            Mail::mailer(config('mail.default'))->to($this->testEmail)->send(new UpgradeConfirmationEmail($subscription, $planName, $billingPeriod));
