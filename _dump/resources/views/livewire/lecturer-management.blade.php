<div>
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div>
            <flux:button wire:click="exportLecturers" icon="arrow-down-tray" variant="primary">Export CSV</flux:button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="md:col-span-2">
            <flux:input wire:model.live="search" icon="magnifying-glass" placeholder="Search lecturers by name or email..." />
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white dark:bg-zinc-900 px-4 py-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col gap-1">
            ...
            <div class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">{{ $lecturers->where('status', 'active')->count() }}</div>
        </div>
        <div class="bg-white dark:bg-zinc-900 px-4 py-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col gap-1">
            <div class="text-xs text-amber-500 font-medium uppercase tracking-wider">Pending</div>
            <div class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">{{ $lecturers->where('status', 'pending')->count() }}</div>
        </div>
        <div class="bg-white dark:bg-zinc-900 px-4 py-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col gap-1">
            <div class="text-xs text-red-500 font-medium uppercase tracking-wider">Rejected</div>
            <div class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">{{ $lecturers->where('status', 'rejected')->count() }}</div>
        </div>
        <div class="bg-white dark:bg-zinc-900 px-4 py-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col gap-1 border-l-4 border-l-zinc-900 dark:border-l-zinc-100">
            ...
            <div class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">{{ $lecturers->total() }}</div>
        </div>
    </div>
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{{ $lecturer->first_name }} {{ $lecturer->last_name }}</div>
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border {{ $color }} uppercase tracking-tight">
                                        {{ $lecturer->status }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        @if($lecturer->status === 'pending')
                                            <button wire:click="openApproveModal({{ $lecturer->id }})" 
                                            ...
                                            <button wire:click="openRejectModal({{ $lecturer->id }})" 
                                        ...
                                        @elseif($lecturer->status === 'active')
                                            <button wire:click="openRemoveModal({{ $lecturer->id }})" 
                                        ...
                                        @endif
                                        <button wire:click="viewLecturerDetails({{ $lecturer->id }})" 
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            @if($selectedLecturer)
                                <p>Are you sure you want to approve <strong>{{ $selectedLecturer->first_name ?? '' }} {{ $selectedLecturer->last_name ?? '' }}</strong>?</p>
            ...
            <div class="modal fade show" id="rejectModal" tabindex="-1" role="dialog" aria-labelledby="rejectModalLabel" aria-hidden="false" style="display: block;" wire:ignore.self>
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            @if($selectedLecturer)
                                <p>Are you sure you want to reject <strong>{{ $selectedLecturer->first_name ?? '' }} {{ $selectedLecturer->last_name ?? '' }}</strong>?</p>
            ...
            <div class="modal fade show" id="removeModal" tabindex="-1" role="dialog" aria-labelledby="removeModalLabel" aria-hidden="false" style="display: block;" wire:ignore.self>
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            @if($selectedLecturer)
                                <p>Are you sure you want to remove <strong>{{ $selectedLecturer->first_name ?? '' }} {{ $selectedLecturer->last_name ?? '' }}</strong>?</p>
                            ...
                            @if($lecturerDetails)
                                <div class="lecturer-detail-grid">
                                    <div class="detail-item">
                                        ...
                                        <span>{{ $lecturerDetails['full_name'] }}</span>
                                    </div>
                                    <div class="detail-item">
                                        <label>Email</label>
                                        <span>{{ $lecturerDetails['email'] }}</span>
                                    </div>
                                    <div class="detail-item">
                                        <label>Phone</label>
                                        <span>{{ $lecturerDetails['phone'] }}</span>
                                    </div>
                                    <div class="detail-item">
                                        <label>Status</label>
                                        <span class="status-badge status-{{ $lecturerDetails['status'] }}">{{ ucfirst($lecturerDetails['status']) }}</span>
                                    </div>
                                    <div class="detail-item">
                                        <label>Registration Date</label>
                                        <span>{{ $lecturerDetails['registration_date'] }}</span>
                                    </div>
                                    <div class="detail-item">
                                        <label>Total Courses</label>
                                        <span>{{ $lecturerDetails['courses_used'] }} / {{ $lecturerDetails['courses_limit'] ?? 'Unlimited' }}</span>
                                    </div>
                                    <div class="detail-item full-width">
                                        <label>Subscription Plan</label>
                                        <span class="plan-badge">
                                            @if($selectedLecturer->school_id)
                                                School Plan: {{ $selectedLecturer->school->activeSubscription?->plan_name ?? 'No School Plan' }}
                                            @else
                                                Individual Plan: {{ $lecturerDetails['subscription_plan'] }}
                                            @endif
                                        </span>
                                    </div>
                                </div>
