<div class="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
    ...
    @if ($hasWeights)
        <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div class="bg-slate-700/50 backdrop-blur rounded-lg p-6 border border-slate-600">
                ...
                <p class="text-3xl font-bold text-white mt-2">{{ number_format($totalExamMarks, 2) }}</p>
            </div>
            <div class="bg-slate-700/50 backdrop-blur rounded-lg p-6 border border-slate-600">
                <p class="text-slate-400 text-sm font-medium">Questions Weighted</p>
                <p class="text-3xl font-bold text-blue-400 mt-2">{{ count($questions) }}</p>
            </div>
            <div class="bg-slate-700/50 backdrop-blur rounded-lg p-6 border border-slate-600">
                ...
                <tbody>
                    @forelse ($questions as $question)
                        <tr class="border-b border-slate-600 hover:bg-slate-600/20 transition">
                            <td class="px-6 py-4">
                                <div>
                                    <p class="font-medium text-slate-100">Q{{ $question['id'] }}</p>
                                    <p class="text-xs text-slate-500 truncate">{{ Str::limit($question['question_text'], 50) }}</p>
                            ...
                            <td class="px-6 py-4 text-center">
                                @if ($editingId === $question['id'])
                                    ...
                                    >
                                @else
                                    <span class="font-bold text-blue-400">{{ isset($weights[$question['id']]) ? number_format($weights[$question['id']]['weight'], 2) : '1.00' }}x</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if ($editingId === $question['id'])
                                    ...
                                    >
                                @else
                                    <span class="font-bold">{{ isset($weights[$question['id']]) ? $weights[$question['id']]['total_marks'] : '1' }}</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="font-bold text-green-400">
                                    @if (isset($weights[$question['id']]))
                                        {{ number_format($weights[$question['id']]['weight'] * $weights[$question['id']]['total_marks'], 2) }}
                                    @else
                                        1.00
                                    @endif
                                </span>
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    <div class="w-24 h-2 bg-slate-600/30 rounded-full overflow-hidden">
                                        <div 
                                            ...
                                            style="width: {{ $this->getWeightPercentage($question['id']) }}%"
                                        ></div>
                                    </div>
                                    <span class="text-sm font-medium text-slate-300 w-12 text-right">
                                        {{ number_format($this->getWeightPercentage($question['id']), 1) }}%
                                    </span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if ($editingId === $question['id'])
                                    <div class="flex gap-2 justify-center">
                                        <button 
                                            wire:click="saveWeight({{ $question['id'] }})"
                                ...
                                @else
                                    <button 
                                        wire:click="editWeight({{ $question['id'] }})"
    ...
    @if ($hasWeights && $totalExamMarks > 0)
                ...
                @foreach ($questions as $question)
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-sm text-slate-300">Q{{ $question['id'] }}: {{ Str::limit($question['question_text'], 40) }}</span>
                            <span class="text-sm font-bold text-slate-100">{{ number_format($this->getWeightPercentage($question['id']), 1) }}%</span>
                        </div>
