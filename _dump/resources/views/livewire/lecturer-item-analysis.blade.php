<div class="p-6 lg:p-10 space-y-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
            <div class="flex items-center gap-2 mb-1">
                <flux:heading size="xl" level="1">Item Analysis</flux:heading>
            ...
            <flux:subheading class="italic">{{ $this->exam->title }}</flux:subheading>
        </div>
        <flux:button wire:click="refreshAnalysis" variant="primary" icon="arrow-path" class="shadow-lg shadow-indigo-500/20">Recompute Metrics</flux:button>
    </div>

    <!-- Summary Stats -->
    @if (!empty($examSummary) && !isset($examSummary['error']))
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 animate-slideIn">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-2xl shadow-sm flex flex-col gap-1 transition-all hover:translate-y-[-2px] hover:shadow-md border-l-4 border-l-zinc-300 dark:border-l-zinc-700">
                ...
                <div class="text-3xl font-bold text-zinc-900 dark:text-zinc-100 italic">{{ $examSummary['total_questions'] ?? 0 }}</div>
            </div>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-2xl shadow-sm flex flex-col gap-1 transition-all hover:translate-y-[-2px] hover:shadow-md border-l-4 border-l-amber-500">
                ...
                <div class="text-3xl font-bold text-amber-500 italic">{{ number_format(($examSummary['average_difficulty'] ?? 0) * 100, 0) }}%</div>
                @php
                ...
                @endphp
                <div class="text-[9px] font-bold uppercase {{ $diffStatus[1] }} tracking-tighter">{{ $diffStatus[0] }}</div>
            </div>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-2xl shadow-sm flex flex-col gap-1 transition-all hover:translate-y-[-2px] hover:shadow-md border-l-4 border-l-indigo-500">
                ...
                <div class="text-3xl font-bold text-indigo-500 italic">{{ number_format(($examSummary['average_discrimination'] ?? 0), 2) }}</div>
                @php
                ...
                @endphp
                <div class="text-[9px] font-bold uppercase {{ $discStatus[1] }} tracking-tighter">{{ $discStatus[0] }}</div>
            </div>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-2xl shadow-sm flex flex-col gap-1 transition-all hover:translate-y-[-2px] hover:shadow-md border-l-4 border-l-rose-500">
                ...
                <div class="text-3xl font-bold text-rose-500 italic">{{ $examSummary['problematic_count'] ?? 0 }}</div>
        ...
        <div class="lg:col-span-4 xl:col-span-3 space-y-6">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-sm overflow-hidden animate-fadeIn">
                <div class="p-5 border-b border-zinc-100 dark:border-zinc-800 space-y-4 bg-zinc-50/50 dark:bg-zinc-900/50">
                    <flux:select wire:model.live="sortBy" label="Sort Metrics">
                        <flux:select.option value="order">Examination Sequence</flux:select.option>
                        <flux:select.option value="difficulty">Difficulty (Rigorous First)</flux:select.option>
                        <flux:select.option value="discrimination">Sensitivity (High First)</flux:select.option>
                    </flux:select>
                    
                    <button wire:click="toggleProblematicFilter" class="w-full h-10 px-4 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all border {{ $showProblematicOnly ? 'bg-rose-50 border-rose-200 text-rose-600 dark:bg-rose-900/20 dark:border-rose-800 dark:text-rose-400' : 'bg-white border-zinc-200 text-zinc-500 hover:bg-zinc-50 dark:bg-zinc-800 dark:border-zinc-700' }}">
                        <i class="fas fa-triangle-exclamation mr-2"></i>
                        {{ $showProblematicOnly ? 'Showing Faulty Items' : 'Show All Items' }}
                    </button>
                </div>

                <div class="max-h-[700px] overflow-y-auto scroll-smooth pr-1">
                    @forelse ($filteredItems as $item)
                        <button wire:click="selectQuestion('{{ $item['question_id'] }}')" class="w-full text-left p-5 border-b border-zinc-100 dark:border-zinc-800/50 transition-all group {{ $selectedQuestionId === (string) $item['question_id'] ? 'bg-indigo-50/30 dark:bg-indigo-900/10 border-l-4 border-l-indigo-500' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/30' }}">
                            <div class="flex items-start justify-between gap-4">
                                <div class="space-y-1">
                                    <div class="text-[11px] font-bold text-zinc-900 dark:text-zinc-100 italic">Reference #{{ $loop->iteration }}</div>
                                    <div class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-tighter border {{ $this->getAssessmentColor($item['assessment']) }} shadow-sm">
                                        {{ $item['assessment'] }}
                                    </div>
                                </div>
                                <div class="text-right flex flex-col gap-1 pl-2">
                                    <div class="text-[10px] font-bold {{ $this->getDifficultyColor($item['difficulty_index']) }} italic uppercase tracking-tighter">D: {{ number_format($item['difficulty_index'] * 100, 0) }}%</div>
                                    <div class="text-[10px] font-bold {{ $this->getDiscriminationColor($item['discrimination_index']) }} italic uppercase tracking-tighter">S: {{ number_format($item['discrimination_index'], 2) }}</div>
                                </div>
                            </div>
                        </button>
                    @empty
        ...
        <div class="lg:col-span-8 xl:col-span-9 space-y-8 sticky top-6">
            @if ($selectedItem)
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-10 rounded-3xl shadow-sm animate-fadeIn space-y-10 relative overflow-hidden group/panel">
                    ...
                    <div>
                        <flux:label class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest px-1">Instrument Prompt</flux:label>
                        <div class="mt-2 text-xl font-bold text-zinc-900 dark:text-zinc-100 leading-relaxed pr-20 italic">
                            "{!! nl2br(e($selectedItem['question_text'])) !!}"
                        </div>
                    </div>

                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div class="bg-white dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 p-8 rounded-3xl shadow-inner border-l-4 {{ $this->getDifficultyColor($selectedItem['difficulty_index']) }}">
                            <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4">Granular Difficulty</div>
                            <div class="flex items-end justify-between">
                                <div class="space-y-1">
                                    <div class="text-5xl font-bold italic {{ $this->getDifficultyColor($selectedItem['difficulty_index']) }}">
                                        {{ number_format($selectedItem['difficulty_index'] * 100, 0) }}%
                                    </div>
                                    <div class="text-[10px] font-bold uppercase text-zinc-400 tracking-tighter">{{ $selectedItem['difficulty_label'] }} Classification</div>
                                </div>
                                <div class="text-right">
                                    <div class="text-[10px] font-bold text-zinc-500 uppercase">Success Rate</div>
                                    <div class="text-lg font-bold text-zinc-900 dark:text-zinc-100">{{ $selectedItem['correct_answer_count'] ?? 0 }} / {{ $selectedItem['total_attempts'] ?? 0 }}</div>
                                </div>
                            </div>
                        </div>

                        <div class="bg-white dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 p-8 rounded-3xl shadow-inner border-l-4 {{ $this->getDiscriminationColor($selectedItem['discrimination_index']) }}">
                            <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4">Correlation Sensitivity</div>
                            <div class="flex items-end justify-between">
                                <div class="space-y-1">
                                    <div class="text-5xl font-bold italic {{ $this->getDiscriminationColor($selectedItem['discrimination_index']) }}">
                                        {{ number_format($selectedItem['discrimination_index'], 2) }}
                                    </div>
                                    <div class="text-[10px] font-bold uppercase text-zinc-400 tracking-tighter">{{ $selectedItem['discrimination_label'] }} Discriminator</div>
                                </div>
                    ...
                    <div class="bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100/50 dark:border-indigo-800/30 p-6 rounded-2xl flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div class="text-sm font-bold text-indigo-700 dark:text-indigo-300 italic mt-1">{{ $selectedItem['assessment'] }}</div>
                            </div>
                        </div>
                    </div>

                    @if (!empty($selectedItem['response_distribution']))
                        <div class="space-y-6">
                            <flux:label class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest px-1">Synthesized Response spread</flux:label>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach ($selectedItem['response_distribution'] as $option => $count)
                                    @php
                                    ...
                                    <div class="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800/50 space-y-2">
                                        <div class="flex items-center justify-between">
                                            <span class="text-xs font-bold text-zinc-900 dark:text-zinc-100 italic">{{ $option }}</span>
                                            <span class="text-[10px] font-bold text-zinc-400 tracking-tighter">{{ $count }} Students</span>
                                        </div>
                                        <div class="h-2 w-full bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden shadow-inner relative">
                                            <div class="h-full bg-gradient-to-r from-indigo-500 to-indigo-400 rounded-full transition-all duration-1000" style="width: {{ $pct }}%"></div>
                                        </div>
                                        <div class="text-[10px] font-bold text-right text-indigo-500 italic">{{ number_format($pct, 1) }}%</div>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    @if ($showRecommendations && !empty($selectedItem['recommendation']))
                        <div class="p-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-3xl animate-slideUp">
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1 space-y-1">
                                    <flux:label class="text-[10px] font-bold text-amber-600/70 dark:text-amber-400 uppercase tracking-widest">Heuristic Optimization</flux:label>
            ...
            @else
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-12 rounded-3xl text-center space-y-4 shadow-sm h-[500px] flex flex-col items-center justify-center animate-fadeIn">
                    ...
                    <div>
                        ...
                        <flux:subheading class="text-center max-w-xs uppercase tracking-widest text-[9px] font-bold mt-2">Initialize instrumentation by selecting an assessment metric from the left registry.</flux:subheading>
                    </div>
                </div>
            @endif
        </div>
    ...
    @if (!empty($examSummary['recommendations']) && $showRecommendations)
        <div class="pt-10 border-t border-zinc-100 dark:border-zinc-800 animate-slideUp">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-[40px] shadow-sm space-y-8">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <flux:heading size="lg">Instrument Oversight Report</flux:heading>
                        <flux:subheading>Strategic refinements for future evaluation cycles.</flux:subheading>
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach ($examSummary['recommendations'] as $recommendation)
                        <div class="p-6 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800 rounded-3xl flex items-start gap-4 transition-all hover:border-indigo-500/30 group">
                            ...
                            <p class="text-xs font-bold text-zinc-600 dark:text-zinc-300 leading-relaxed italic">{{ $recommendation }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    <div class="flex justify-center pt-8">
        <flux:button wire:click="toggleRecommendations" variant="ghost" size="sm" icon="{{ $showRecommendations ? 'eye-slash' : 'eye' }}" class="text-[10px] font-bold uppercase tracking-widest">
            {{ $showRecommendations ? 'Conceal Heuristics' : 'Divulge Insights' }}
        </flux:button>
    </div>
</div>
