...
<div class="admin-page">
    <div class="page-header">
        <div class="header-top">
            ...
            <a href="{{ route('team.content.pages.create') }}" class="btn btn-primary">+ New Page</a>
        </div>
    </div>
        ...
        <table class="pages-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="badge badge-{{ $page->status }}">{{ ucfirst($page->status) }}</span></td>
                    <td>{{ $page->views ?? 0 }}</td>
                    <td>{{ $page->created_at->format('M d, Y') }}</td>
                    <td>{{ $page->updated_at->format('M d, Y') }}</td>
                    <td>
                        <a href="{{ route('team.content.pages.edit', $page) }}" class="btn-icon" title="Edit">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path>
                            </svg>
                        </a>
                        <a href="/pages/{{ $page->slug }}" class="btn-icon" title="View" target="_blank">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                                <circle cx="12" cy="12" r="3"></circle>
                            </svg>
                        </a>
                        <form action="{{ route('team.content.pages.destroy', $page) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr><td colspan="7" class="empty-state">No pages found - <a href="{{ route('team.content.pages.create') }}" style="color: #60a5fa;">Create your first page!</a></td></tr>
                @endforelse
        </table>
    </div>
