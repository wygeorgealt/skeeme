    ...
    <div class="filter-section">
        <form method="GET" class="filter-form">
            <div class="filter-row">
                ...
                <a href="{{ route('team.support.tickets.index') }}" class="btn btn-secondary">Clear</a>
            </div>
        </form>
    </div>
        ...
        <table class="tickets-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="ticket-subject">{{ $ticket->title }}</span></td>
                    ...
                    <td class="time-cell">{{ $ticket->created_at->diffForHumans() }}</td>
                    <td>
                        <button class="btn-icon" onclick="openTicketModal('{{ $ticket->id }}', '{{ $ticket->title }}')" title="Respond">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
                            </svg>
                        </button>
                        @if($ticket->status !== 'resolved')
                        <form action="{{ route('team.support.tickets.resolve', $ticket) }}" method="POST" style="display:inline;">
                            @csrf
