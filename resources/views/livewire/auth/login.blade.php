    ...
    <div class="flex flex-col gap-3">
        ...
        <form method="POST" action="{{ route('login.store') }}" class="flex flex-col gap-4">
            ...
            <div class="relative">
                ...
                @if (Route::has('password.request'))
                    <flux:link class="absolute bottom-12 text-xs font-semibold end-0 no-underline text-indigo-600 hover:text-indigo-700" :href="route('password.request')">
                        {{ __('Forgot?') }}
                    </flux:link>
                @endif
            </div>
            ...
            <div class="flex items-center justify-end pt-1">
                <flux:button variant="primary" type="submit" class="w-full !rounded-xl !py-3 font-bold shadow-lg shadow-indigo-100" data-test="login-button">
                    {{ __('Log in') }}
                </flux:button>
            </div>
        ...
        @if (Route::has('register'))
            <div class="space-x-1 text-center text-sm text-slate-500">
                ...
                <flux:link class="no-underline font-bold text-indigo-600" :href="route('register')">{{ __('Sign up') }}</flux:link>
            </div>
        @endif
    </div>
</x-layouts.auth>
