<div class="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
    ...
    <div class="max-w-6xl mx-auto mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-slate-400">{{ $exam->title }}</p>
            </div>
            <button 
    ...
    @else
        ...
        <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <div class="bg-slate-700/50 backdrop-blur rounded-lg p-6 border border-slate-600">
                ...
                <p class="text-2xl font-bold text-blue-400 mt-2 capitalize">{{ $adaptiveType }}</p>
            </div>
            <div class="bg-slate-700/50 backdrop-blur rounded-lg p-6 border border-slate-600">
                <p class="text-slate-400 text-sm font-medium">Question Pools</p>
                <p class="text-2xl font-bold text-indigo-400 mt-2">{{ count($pools) }}</p>
            </div>
            <div class="bg-slate-700/50 backdrop-blur rounded-lg p-6 border border-slate-600">
                <p class="text-slate-400 text-sm font-medium">Routing Rules</p>
                <p class="text-2xl font-bold text-purple-400 mt-2">{{ count($rules) }}</p>
            </div>
            <div class="bg-slate-700/50 backdrop-blur rounded-lg p-6 border border-slate-600">
                <p class="text-slate-400 text-sm font-medium">Total Questions</p>
                <p class="text-2xl font-bold text-green-400 mt-2">{{ count($allQuestions) }}</p>
            </div>
        </div>
                ...
                <div class="bg-slate-700/30 backdrop-blur rounded-lg border border-slate-600 p-6">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center gap-2">
                            <span>@if (count($pools) > 0) ✓ @else ✗ @endif</span>
                            <span class="text-slate-300">{{ count($pools) }} question pool(s) created</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span>@if (count($rules) > 0) ✓ @else ✗ @endif</span>
                            <span class="text-slate-300">{{ count($rules) }} routing rule(s) defined</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span>@if (count($pools) > 0 && count($rules) > 0) ✓ @else ✗ @endif</span>
                        ...
                        <div class="space-y-3">
                            @foreach ($pools as $pool)
                                <div class="bg-slate-600/20 rounded-lg p-4 border-l-4 border-l-indigo-500">
                                        ...
                                        <div>
                                            ...
                                            <div class="flex gap-2 mt-1">
                                                <span class="px-2 py-1 bg-slate-600/30 rounded text-xs text-slate-400">
                                                    @switch($pool['difficulty'])
                                                        @case('easy') 🟢 Easy
                                                        @case('moderate') 🟡 Moderate
                                                        @case('difficult') 🟠 Difficult
                                                        @case('very_difficult') 🔴 Very Difficult
                                                        @default {{ $pool['difficulty'] }}
                                                    @endswitch
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    @if ($pool['description'])
                                        <p class="text-xs text-slate-400 mb-2">{{ $pool['description'] }}</p>
                                    @endif
                                    <button 
                                        wire:click="$set('selectedPoolForQuestions', {{ $pool['id'] }}); $toggle('showAssignModal')"
                        ...
                        <div class="space-y-3">
                            @foreach ($rules as $rule)
                                <div class="bg-slate-600/20 rounded-lg p-4 border-l-4 border-l-green-500">
                                    <div class="flex items-start justify-between mb-2">
                                        <div>
                                            <p class="font-bold text-white">{{ $rule['rule_name'] }}</p>
                                            <p class="text-xs text-slate-400 mt-1">
                                                After Q{{ $rule['question_sequence'] }}: If performance {{ $rule['operator'] }} {{ number_format($rule['performance_threshold'] * 100, 0) }}%
                                            </p>
                                            <p class="text-xs text-slate-400">→ Show {{ $rule['questions_to_present'] }} from pool: <strong>{{ $rule['target_pool_name'] ?? 'Unknown' }}</strong></p>
                                        </div>
                                        <button 
    ...
    @if ($showAssignModal && $selectedPoolForQuestions)
                ...
                <div class="bg-slate-700/50 rounded-lg p-4 max-h-96 overflow-y-auto mb-6">
                    @forelse ($allQuestions as $question)
                        <label class="flex items-center gap-3 p-2 hover:bg-slate-600/20 rounded cursor-pointer">
                            ...
                            >
                            <div class="flex-1">
                                <p class="font-medium text-slate-100">Q{{ $question['id'] }}: {{ Str::limit($question['question_text'], 50) }}</p>
                                <p class="text-xs text-slate-500">{{ $question['question_type'] }}</p>
                            </div>
                        </label>
                    @empty
