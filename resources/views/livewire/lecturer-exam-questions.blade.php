<div class="p-6 lg:p-10 space-y-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div class="space-y-1">
            <div class="flex items-center gap-2">
                <flux:heading size="xl" level="1">Manage Exam Questions</flux:heading>
            ...
            <flux:subheading class="flex items-center gap-2">
                <span class="font-bold text-zinc-900 dark:text-zinc-100 italic">{{ $exam->title }}</span>
                <span class="text-zinc-400">•</span>
                <span class="text-xs uppercase tracking-tighter font-bold text-zinc-500">{{ $exam->course->name }}</span>
            </flux:subheading>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4 rounded-2xl shadow-sm flex items-center gap-6">
            <div class="text-center px-4 border-r border-zinc-100 dark:border-zinc-800">
                ...
                <div class="text-2xl font-bold text-indigo-500 italic font-mono">{{ $this->totalMarks }}</div>
            </div>
        ...
        <div class="w-full lg:w-64 flex-shrink-0">
            <div class="flex flex-col gap-2 sticky top-6">
                <button wire:click="$set('activeTab', 'review')" class="group flex items-center gap-3 p-4 rounded-2xl transition-all border {{ $activeTab === 'review' ? 'bg-zinc-900 border-zinc-900 text-white shadow-lg dark:bg-white dark:border-white dark:text-zinc-900' : 'bg-white border-zinc-200 text-zinc-600 hover:border-zinc-900 dark:bg-zinc-900 dark:border-zinc-800 dark:text-zinc-400 dark:hover:border-white shadow-sm' }}">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center border {{ $activeTab === 'review' ? 'bg-white/10 border-white/20' : 'bg-zinc-50 border-zinc-100 dark:bg-zinc-800 dark:border-zinc-700' }}">
                ...
                <button wire:click="$set('activeTab', 'manual')" class="group flex items-center gap-3 p-4 rounded-2xl transition-all border {{ $activeTab === 'manual' ? 'bg-zinc-900 border-zinc-900 text-white shadow-lg dark:bg-white dark:border-white dark:text-zinc-900' : 'bg-white border-zinc-200 text-zinc-600 hover:border-zinc-900 dark:bg-zinc-900 dark:border-zinc-800 dark:text-zinc-400 dark:hover:border-white shadow-sm' }}">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center border {{ $activeTab === 'manual' ? 'bg-white/10 border-white/20' : 'bg-zinc-50 border-zinc-100 dark:bg-zinc-800 dark:border-zinc-700' }}">
                ...
                <button wire:click="$set('activeTab', 'bank')" class="group flex items-center gap-3 p-4 rounded-2xl transition-all border {{ $activeTab === 'bank' ? 'bg-zinc-900 border-zinc-900 text-white shadow-lg dark:bg-white dark:border-white dark:text-zinc-900' : 'bg-white border-zinc-200 text-zinc-600 hover:border-zinc-900 dark:bg-zinc-900 dark:border-zinc-800 dark:text-zinc-400 dark:hover:border-white shadow-sm' }}">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center border {{ $activeTab === 'bank' ? 'bg-white/10 border-white/20' : 'bg-zinc-50 border-zinc-100 dark:bg-zinc-800 dark:border-zinc-700' }}">
                ...
                @if($this->canAccessAI)
                <button wire:click="$set('activeTab', 'ai_generator')" class="group flex items-center gap-3 p-4 rounded-2xl transition-all border {{ $activeTab === 'ai_generator' ? 'bg-zinc-900 border-zinc-900 text-white shadow-lg dark:bg-white dark:border-white dark:text-zinc-900' : 'bg-white border-zinc-200 text-zinc-600 hover:border-zinc-900 dark:bg-zinc-900 dark:border-zinc-800 dark:text-zinc-400 dark:hover:border-white shadow-sm' }}">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center border {{ $activeTab === 'ai_generator' ? 'bg-white/10 border-white/20' : 'bg-zinc-50 border-zinc-100 dark:bg-zinc-800 dark:border-zinc-700' }}">
        ...
        <div class="flex-1 min-w-0">
            <div class="relative min-h-[500px]">
                ...
                <div class="animate-fadeIn">
                    @include('livewire.partials.exam-questions-review')
                    @include('livewire.partials.exam-questions-manual')
                    @include('livewire.partials.exam-questions-bank')
                    @include('livewire.partials.exam-questions-ai')
                </div>
            </div>
        </div>
    </div>
    ...
    @if($showQuestionPreview && $previewQuestion)
        @include('livewire.partials.question-preview-modal')
    @endif
