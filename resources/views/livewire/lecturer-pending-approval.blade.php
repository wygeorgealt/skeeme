<x-layouts.auth title="Pending Approval">
    <div class="flex flex-col items-center text-center gap-6" data-aos="zoom-in">
        ...
        <div class="relative">
            ...
            <div
                class="size-24 rounded-3xl bg-indigo-600 flex items-center justify-center text-white text-4xl shadow-2xl shadow-indigo-200 relative z-10">
        ...
        <div class="flex flex-col gap-2">
            ...
            <p class="text-slate-500 font-medium">Your request to join <span
                    class="text-indigo-600 font-bold decoration-wavy underline decoration-indigo-200 underline-offset-4">{{ $lecturer->school->name ?? 'your institution' }}</span>
                is being reviewed.</p>
        </div>
        ...
        <div class="flex flex-col gap-4 w-full pt-4">
            <flux:button href="{{ route('home') }}" variant="ghost"
                class="w-full !rounded-xl !py-4 font-bold border border-slate-200">
                Back to Homepage
            </flux:button>
            <flux:link href="mailto:noreply@contact.skeeme.com"
                class="text-xs text-slate-400 font-bold uppercase tracking-widest no-underline hover:text-indigo-600 transition-colors">
                Contact Support
            </flux:link>
        </div>
    </div>
