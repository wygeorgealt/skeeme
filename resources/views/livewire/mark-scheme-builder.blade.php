    ...
    @else
        ...
        <div class="max-w-6xl mx-auto bg-slate-700/30 backdrop-blur rounded-lg border border-slate-600 p-6 mb-8">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-bold text-white">{{ $scheme->name }}</h2>
                        ...
                        <div class="space-y-3 max-h-96 overflow-y-auto">
                            @forelse ($items as $item)
                                <div class="bg-slate-600/20 border-l-4 border-l-indigo-500 rounded-lg p-4">
                                    <div class="flex items-start justify-between mb-2">
                                        <div>
                                            <p class="font-bold text-white">
                                                {{ $item['level_name'] }} 
                                                <span class="text-green-400">{{ $item['marks_awarded'] }}/{{ $totalMarks }} marks</span>
                                            </p>
                                            <p class="text-slate-400 text-xs mt-1">Level {{ $item['level'] }}</p>
                                        </div>
                                    ...
                                    <p class="text-slate-300 text-sm mb-2">{{ $item['criteria'] }}</p>
                                    @if ($item['examples'])
                                        <details class="text-xs">
                                            <summary class="text-slate-400 cursor-pointer hover:text-slate-300">Show examples</summary>
                                            <p class="text-slate-400 mt-2 pl-2 border-l border-slate-500">{{ $item['examples'] }}</p>
                                        </details>
                                    @endif
                                </div>
                            @empty
                        ...
                        <div class="space-y-2">
                            @foreach ($assignedQuestions as $question)
                                <div class="flex items-center justify-between p-3 bg-slate-600/20 rounded border border-slate-600">
                                    <div>
                                        <p class="font-medium text-slate-100">Q{{ $question->id }}: {{ Str::limit($question->question_text, 60) }}</p>
                                        <p class="text-xs text-slate-500">{{ $question->exam->title ?? 'N/A' }}</p>
                                    </div>
                                    <button 
                ...
                <div class="max-w-6xl mx-auto bg-slate-700/30 backdrop-blur rounded-lg border border-slate-600 p-6 mb-8">
                    ...
                    @if ($schemeInstructions)
                        <div class="bg-blue-600/20 border-l-4 border-l-blue-500 rounded-lg p-4 mb-6">
                            ...
                            <p class="text-slate-300 whitespace-pre-wrap">{{ $schemeInstructions }}</p>
                        </div>
                    @endif
                            ...
                            <div class="border border-slate-600 rounded-lg p-4">
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="font-bold text-lg">
                                        <span class="text-green-400">{{ $item['marks_awarded'] }}/{{ $totalMarks }}</span>
                                        <span class="text-white ml-2">{{ $item['level_name'] }}</span>
                                    </h4>
                                    <span class="text-sm text-slate-400">{{ number_format(($item['marks_awarded'] / $totalMarks) * 100, 0) }}%</span>
                                </div>
                                <p class="text-slate-300 mb-2">{{ $item['criteria'] }}</p>
                                @if ($item['examples'])
                                    <div class="bg-slate-600/30 rounded p-3 mt-2 border-l-2 border-l-slate-400">
                                        ...
                                        <p class="text-sm text-slate-300">{{ $item['examples'] }}</p>
                                    </div>
                                @endif
                            </div>
                        @empty
