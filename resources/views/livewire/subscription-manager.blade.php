                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <dd class="text-lg font-semibold text-gray-900">{{ $status['plan'] }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Days Remaining</dt>
                                <dd class="text-lg font-semibold text-gray-900">{{ $status['days_remaining'] }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Students</dt>
                                <dd class="text-lg font-semibold text-gray-900">
                                    {{ $status['current_students'] }} / {{ $status['student_limit'] ?: 'Unlimited' }}
                                </dd>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($status['features'] as $feature => $enabled)
                            <div class="flex items-center">
                                ...
                                @endif
                                <span class="ml-2 text-sm text-gray-700">{{ ucwords(str_replace('_', ' ', $feature)) }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
    ...
    @if($showUpgradeModal)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" id="upgrade-modal">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Upgrade to {{ $selectedPlan }}</h3>
                    <div class="mb-4">
